class TestFont
  ...
  def test_setters
    ...
    s = Object.new
    def s.to_int; 50; end
    assert_same(f.send(:name=, o), o)
    assert_equal(f.send(:size=, s), s)
    assert_equal(f.send(:bold=, true), true)
    assert_equal(f.send(:italic=, true), true)
    assert_equal(f.send(:outline=, false), false)
    assert_equal(f.send(:shadow=, true), true)
    c = Color.new(1.5, 2.5, 3.5, 4.5)
    assert_same(f.send(:color=, c), c)
    assert_equal(f.color, c)
    assert_not_same(f.color, c)
    c = Color.new(11.5, 12.5, 13.5, 14.5)
    assert_same(f.send(:out_color=, c), c)
    assert_equal(f.out_color, c)
    assert_not_same(f.out_color, c)
    o = Object.new
    s = Object.new
    def s.to_int; 50; end
    assert_equal(Font.send(:default_name=, o), o)
    assert_equal(Font.send(:default_size=, s), s)
    assert_equal(Font.send(:default_bold=, true), true)
    assert_equal(Font.send(:default_italic=, true), true)
    assert_equal(Font.send(:default_outline=, false), false)
    assert_equal(Font.send(:default_shadow=, true), true)
    c = Color.new(1.5, 2.5, 3.5, 4.5)
    assert_same(Font.send(:default_color=, c), c)
    assert_equal(Font.default_color, c)
    # assert_not_same(Font.default_color, c)
    assert_same(Font.default_color, c)
    c = Color.new(11.5, 12.5, 13.5, 14.5)
    assert_same(Font.send(:default_out_color=, c), c)
    ...
    f = Font.new
    assert(FalseClass === (f.send(:bold=, nil)))
    assert(FalseClass === f.bold)
    assert(TrueClass === (f.send(:bold=, 0)))
    assert(TrueClass === f.bold)
    assert(FalseClass === (f.send(:italic=, nil)))
    assert(FalseClass === f.italic)
    assert(TrueClass === (f.send(:italic=, 0)))
    assert(TrueClass === f.italic)
    assert(FalseClass === (f.send(:shadow=, nil)))
    assert(FalseClass === f.shadow)
    assert(TrueClass === (f.send(:shadow=, 0)))
    assert(TrueClass === f.shadow)
    assert(FalseClass === (f.send(:outline=, nil)))
    assert(FalseClass === f.outline)
    assert(TrueClass === (f.send(:outline=, 0)))
    assert(TrueClass === f.outline)
    assert_raise(TypeError) { f.color = Object.new }
    assert_raise(TypeError) { f.out_color = Object.new }
    assert(NilClass === (Font.send(:default_bold=, nil)))
    assert(NilClass === Font.default_bold)
    assert(Integer === (Font.send(:default_bold=, 0)))
    assert(Integer === Font.default_bold)
    assert(NilClass === (Font.send(:default_italic=, nil)))
    assert(NilClass === Font.default_italic)
    assert(Integer === (Font.send(:default_italic=, 0)))
    assert(Integer === Font.default_italic)
    assert(NilClass === (Font.send(:default_shadow=, nil)))
    assert(NilClass === Font.default_shadow)
    assert(Integer === (Font.send(:default_shadow=, 0)))
    assert(Integer === Font.default_shadow)
    assert(NilClass === (Font.send(:default_outline=, nil)))
    assert(NilClass === Font.default_outline)
    assert(Integer === (Font.send(:default_outline=, 0)))
